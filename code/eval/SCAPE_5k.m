...
load('models/SCAPE_5k/SCAPE_5k_test_on_scape_5k_sample_new');
set_basis = basis;

data_dir = 'data/SCAPE_5k/off/';                       
data_name_list = ls ([data_dir,'*.off']);

data_name_list = data_name_list(52:end,:);
num_of_data = 20;
[V,Faces] = load_raw_data(data_dir,data_name_list);

vts_5k = {};
for i = 1:num_of_data
    vts = load(['data/SCAPE_5k/corres/mesh',num2str(i-1+52,'%03d'),'.vts']);
    vts_5k{1,i} = vts;
end
